...
class DeployTest extends BltProjectTestBase {
  ...
  public function testBltDeployBuild() {
    ...
    $this->assertFileDoesNotExist($this->deploy_dir . '/docroot/sites/default/settings/local.settings.php');
    ...
    $this->assertFileDoesNotExist($this->deploy_dir . '/docroot/LICENSE.txt');

    // Ensure non-required files are not in deploy dir.
    $this->assertFileExists($this->deploy_dir . '/blt/blt.yml');
    $this->assertFileDoesNotExist($this->deploy_dir . '/tests');
