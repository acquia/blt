...
acsf_deploy() {
  ...
  blt deploy:update  -D environment $target_env -D drush.uri "$uri" -v
  if [ $? -ne 0 ]; then
...
ace_deploy() {
  ...
  blt deploy:update -D environment $target_env -v
  if [ $? -ne 0 ]; then
      echo "Update errored."
      exit 1
  fi
...
deploy_install() {
  ...
  blt deploy:drupal:install -D environment=$target_env -v
  if [ $? -ne 0 ]; then
